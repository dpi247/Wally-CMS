...
function soirmag_tree_output($tree, $firstpass=1) { 
  ...
  foreach ($tree as $data) {
    ...
    if ($data['below']) {
      ...
      $output .= theme_soirmag_item($link, $data['link']['has_children'], $sub_menu, $data['link']['in_active_trail'], $extra_class, $c);
    }
    else {
      $output .= theme_soirmag_item($link, $data['link']['has_children'], '', $data['link']['in_active_trail'], $extra_class, $c);
    }
  $c++;
  }
