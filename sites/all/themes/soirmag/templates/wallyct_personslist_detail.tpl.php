  ...
  function wally_tpl_persons_detail($persons) {
    ...
    foreach ($persons as $person) {
        ...
        if (isset($person->field_persontaxonomy)) {
          $title = "<a href='".base_path().$taxo_path."'>".$person->title."</a>";
        } else {
          $title = $person->title;
        }
        ...
        if (isset($person->field_main_picture)) {
            ...
            $content .= "<tr class='".(($i++%2) ? "even" : "odd")."'><td><img src='/".path_to_theme()."/mediastore/default_person_pic.jpg' width='50' align='left'></td><td>".$person->field_personfirstname[0]["value"]."</td><td>".$person->field_personlastname[0]["value"]."</td></tr>";
        }
...
<div id="persons" class="grid_6">
  <h2>test Personnes</h2>
  <?php print wally_tpl_persons_detail($persons); ?>
</div>
