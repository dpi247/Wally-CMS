...
class MiniXMLDoc {
	...
	*/
	function fromString (&$XMLString)
	{
		...
		/* fromString XMLSTRING - PATCH
		** 
		** 08 Oct 2010 - Didier Lahousse - Audaxis ( DLA )  
		** 
		** Add: $XMLString=preg_replace('!<\s*([a-zA-Z0-9_-]+)(\s*)([^>]*)/>!','<$1$2$3></$1>',$XMLString); 
		** 
		** This PATCH correct the error when handling an empty element without closing tag when  
		** it's the 1st element in a list.
		** 
		** <document>
		**  <Channels>
		** 	 <Channel Name="one" />
		** 	 <Channel Name="two">Not empty</Channel>
		** </Channels>
		** </document>
		**   
		*/
		$XMLString=preg_replace('!<\s*([a-zA-Z0-9_-]+)(\s*)([^>]*)/>!','<$1$2$3></$1>',$XMLString);
