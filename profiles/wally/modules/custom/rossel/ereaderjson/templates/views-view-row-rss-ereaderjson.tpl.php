...
ereaderjson_build_embedded_links(&$node);
...
if ($field_mainstory_nodes->field_textbody[0]['value'] != '' & $field_mainstory_nodes->field_textbody[0]['value'] != NULL){
  ...
  $texte_article = _ereaderjson_clear_paragraph($texte_article);
}
...
foreach($node->field_embededobjects_nodes as $one){
		...
		switch($one->type){
			...
			case "wally_photoobject":
				$embed_one = ereaderjson_get_photo_infos_and_display($one);
				...
				$photoobject['crop']['medium']= array(					 				 				 				
					'height'=>'300',
					'width'=>'400',
					'url'=> $base_url."/sites/default/files/imagecache/ereadermedium/".$embed_one["fullpath"]
				);
				$photoobject['crop']['large']= array(
					'height'=>'780',
					'width'=>'1024',
					'url'=> $base_url."/sites/default/files/imagecache/ereaderlarge/".$embed_one["fullpath"]
				);
				$photoobject['crop']['thumb']= array(
					'height'=>'79',
					'width'=>'102',
					'url'=> $base_url."/sites/default/files/imagecache/ereaderthumbnail/".$embed_one["fullpath"]
				);
    	...
    	case "wally_videoobject":
    		...
    		$embed_one = ereaderjson_get_video_infos_and_display($one);
    	...
    	case "wally_audioobject":
				$embed_one = ereaderjson_get_audio_infos_and_display($one);
				$json['relatedObjects'][] = array(
				'nid'=> $embed_one["nid"],
    	...
    	case "wally_digitalobject":
				$embed_one = ereaderjson_get_digitalobject_infos_and_display($one);
				$dummy = array(
				'nid'=> $embed_one["nid"],
